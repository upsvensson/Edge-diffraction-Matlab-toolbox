...
% EDmain_convexESIEtime - Calculates the specular and edge diffraction IRs 
...
if nargin < 6
    ...
    if nargin < 5
        error('ERROR: The first five input parameters to EDmain_convexESIEtime must be specified')  
    end
end
...
[geoinputdata,Sinputdata,Rinputdata,envdata,controlparameters,filehandlingparameters] = EDcheckinputstructs(geoinputdata,Sinputdata,Rinputdata,envdata,controlparameters,filehandlingparameters,2);
...
if filehandlingparameters.showtext >= 1
	disp('    ');disp('####################################################################')
              disp(['#  EDmain_convexESIEtime, v. ',num2str(EDversionnumber),' (',lastsavedate,')'])
...
if filehandlingparameters.savelogfile == 1
    ...
    fwrite(fid,['#  EDmain_convexESIEtime, v. ',num2str(EDversionnumber),' (',lastsavedate,')',lineending],'char');
...
if controlparameters.difforder > 1 && controlparameters.docalcir == 1
    disp('Sorry, higher-order impulse responses have not been implemented yet')
%     if filehandlingparameters.showtext >= 1	
...
%     end
    ...
    if filehandlingparameters.showtext >= 1	
        disp(['   Skipping the TD integral equation, since difforder = ',int2str(controlparameters.difforder),' (or docalcir was set to 0)'])
    end
    timingstruct.integralequation = [0 0 0 0 0];
    if Sinputdata.doaddsources == 1 || nsources == 1
        irinteqdiff = zeros(1,nreceivers);
    else
        irinteqdiff = zeros(nreceivers,nsources);        
    end
    extraoutputdata = [];
    desiredname = [filehandlingparameters.outputdirectory,filesep,filehandlingparameters.filestem,'_irinteq.mat'];
    eval(['save ',desiredname,' irinteqdiff extraoutputdata timingstruct'])    
    if filehandlingparameters.savelogfile == 1
